...
public class MemoryRepositoryDouble implements MemoryRepository {
    /* The MemoryRepositoryDouble implements MemoryRepository so it can be injected into MemoryService. It stores three
    memories in an ArrayList<Memory> and uses them to decide if a memory is update-able or delete-able. Additions are
    allowed without question since we really don't care about behavior in the repository. Our goal is to test
    MemoryService code. */
